									...
									<table class="table table-striped" style="margin-bottom:10">
										...
										<tbody>
											<tr v-for="(item, key) in items.data" :index="key">
												...
												<td>{{ item.name }} </td>
												<td>
													<span class="badge badge-primary mr-1" v-for="permission in item.permissions">{{ permission.name }}</span>
												</td>
												<td>
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title"><i v-if="modal.icon" :class="modal.icon"></i> {{ modal.title }} Role</h5>
					...
					<form @submit.prevent="submit()">
						<div class="modal-body">
							...
							<div class="form-group">
								...
								<br>
								<select name="permissions[]" v-model="form.permissions" class="form-control select2" style="width: 100%;" multiple="multiple">
...
export default {
	...
	data () {
		return {
			url: '/dashboard/api/management/role',
			error: '',
			perPage: 10,
			loadings: {
				items: true,
	...
	mounted () {
		...
		$('.select2').change(function () {
			self.form.permissions = $('.select2').val();
		});
	},

	methods: {
		getPermissions () {
			axios.get('/dashboard/api/management/permission?no_pagging=true')
			.then(response => {
				this.permissions = response.data;
			})
			.catch(error => {
				toastr.error('Filed to get permission!','Error', {
					progressBar: true,
					timeOut: 3000
				})
			});
		...
		edit (data) {
			...
			data.permissions.forEach(d => {
				this.form.permissions.push(d.name);
			});

			console.log('edit', this.form.permissions);
			this.form.errors = [];
			$('#modal').modal('show');
		},
